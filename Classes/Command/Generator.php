...
//		$output[] = '';
	...
	protected function getKeyMarkers($config) {
		$key = $config['key'];
	...
	protected function buildClassProperties($columns) {
	...
	protected function buildClassAccessors($columns) {
		foreach ($columns as $column => $config) {
	...
	protected function buildConstructor($columns) {
		foreach ($columns as $column => $config) {
