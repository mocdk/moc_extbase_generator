<?php
class Tx_MocExtbaseGenerator_Utility_Inflector {
	 ...
	 *	Tx_MocExtbaseGenerator_Utility_Inflector::rules('plural', array('/^(inflect)or$/i' => '\1ables'));
	 *	Tx_MocExtbaseGenerator_Utility_Inflector::rules('plural', array(
	...
	public static function camelize($lowerCaseAndUnderscoredWord) {
		if (!isset(self::$_camelize[$lowerCaseAndUnderscoredWord])) {
			self::$_camelize[$lowerCaseAndUnderscoredWord] = str_replace(" ", "", ucwords(str_replace("_", " ", $lowerCaseAndUnderscoredWord)));
		}
	...
	public static function underscore($camelCasedWord) {
		if (!isset(self::$_underscored[$camelCasedWord])) {
			self::$_underscored[$camelCasedWord] = strtolower(preg_replace('/(?<=\\w)([A-Z])/', '_\\1', $camelCasedWord));
		}
	...
	public static function humanize($lowerCaseAndUnderscoredWord) {
		if (!isset(self::$_humanize[$lowerCaseAndUnderscoredWord])) {
			self::$_humanize[$lowerCaseAndUnderscoredWord] = ucwords(str_replace("_", " ", $lowerCaseAndUnderscoredWord));
		}
	...
	public static function tableize($className) {
		if (!isset(self::$_tableize[$className])) {
			self::$_tableize[$className] = Tx_MocExtbaseGenerator_Utility_Inflector::pluralize(Tx_MocExtbaseGenerator_Utility_Inflector::underscore($className));
		}
	...
	public static function classify($tableName) {
		if (!isset(self::$_classify[$tableName])) {
			self::$_classify[$tableName] = Tx_MocExtbaseGenerator_Utility_Inflector::camelize(Tx_MocExtbaseGenerator_Utility_Inflector::singularize($tableName));
		}
	...
	public static function variable($string) {
		if (!isset(self::$_variable[$string])) {
			$string = Tx_MocExtbaseGenerator_Utility_Inflector::camelize(Tx_MocExtbaseGenerator_Utility_Inflector::underscore($string));
			$replace = strtolower(substr($string, 0, 1));
			self::$_variable[$string] = preg_replace('/\\w/', $replace, $string, 1);
		}
